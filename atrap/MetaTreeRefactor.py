...
class MetaTree(object):
            ...
            for inferral_strategy_generator in self.inferral_strategy_generators:
                ...
                for inferral_strategy in inferral_strategy_generator(tiling, basis=self.basis, basis_partitioning=self._basis_partitioning):
                    ...
                    if inferred_tiling in self._inferral_cache:
                        inferred_tiling = self._inferral_cache[inferred_tiling]
                        fully_inferred = True
                        break
                    else:
                        semi_inferred_tilings.append(inferred_tiling)
            for semi_inferred_tiling in semi_inferred_tilings:
                self._inferral_cache[semi_inferred_tiling] = inferred_tiling
                '''Clean up the cache'''
    ...
    def _basis_partitioning(self, tiling, length, basis):
        ...
        if cache is None:
            self._basis_partitioning_cache[tiling] = {}
            cache = self._basis_partitioning_cache[tiling]
        else:
