...
def binary_pattern(tiling, basis, **kwargs):
    ...
    if isinstance(block, PositiveClass) and k:
        for patt in PermSet.avoiding(basis).of_length(k):
            if patt != Perm((0, 1, 2)):
                continue
            cclass = list(filter(lambda x: is_binary(x, basis),
                map(lambda m: MeshPatt.unrank(patt, m), coincidence_classification[patt])))
            maximal = list()
            last = None
            ...
            # print(sorted(cclass))
            # print(maximal)
            for mpatt in maximal:
                print(mpatt)
                print()
                print(tiling_from_mesh_pattern(mpatt, block.perm_class))
                print(patt)
