            ...
            for sub_tiling in strategy:
                ...
                if child_or_node is None:
                    ...
                    for verification_strategy_generator in verification_strategies:
                        for _ in verification_strategy_generator(tiling,
                                                                 basis=basis,
...
def find_good_splittings(tiling, basis, basis_partitioning=None, built=[]):
    ...
    subsets_to_check = itertools.chain.from_iterable(itertools.combinations(cells_to_check, r)
                                                     for r in range(0, len(cells_to_check)+1))

    good_splittings = []
    min_length = min([len(P) for P in basis])
    verification_length = (max([len(P) for P in basis])
                           + tiling.total_points
                           + sum(1 for _, block in tiling.non_points if isinstance(block, PositiveClass)))
    ...
    for subset in subsets_to_check:
        ...
        for length_to_check in range(min_length, verification_length+1):
            ...
            for basis_element in containing_perms.keys():
                ...
                for position_set in containing_perms[basis_element]:
                    ...
                    permize = (Perm(standardize([entry for entry in part_perm if entry != -1]))
                               for part_perm in partition_split)
        ...
        if good_partition:
            good_splittings.extend(find_good_splittings(tiling,
                                                        basis,
                                                        basis_partitioning=basis_partitioning,
                                                        built=built+[new_part]))
