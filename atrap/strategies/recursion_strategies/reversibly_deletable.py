...
def reversibly_deletable_points_joined(tiling, basis, current_cell=None, occurrences_by_perm=None, path=None, basis_partitioning=None, **kwargs):
    '''This is the same as reversibly_deletable_points, but the
    points are returned in one tiling'''
    if current_cell is None:
        '''Take the smallest cell in the tiling'''
        current_cell = (-1,-1)
    if occurrences_by_perm is None:
    ...
    for cell in tiling.point_cells:
        ...
        if reversibly_deletable:
            ...
            strategy = [ Tiling(new_tiling_dict), points ]
            yield RecursiveStrategy(formal_step, strategy, [tiling._back_map for tiling in strategy] )
            for recursive_strategy in reversibly_deletable_points_joined(tiling, basis, cell, new_occurrences_by_perm, path):
                yield recursive_strategy
            path.pop()

def reversibly_deletable_cells_joined(tiling, basis, current_cell=None, occurrences_by_perm=None, path=None, basis_partitioning=None, **kwargs):
    ...
    for cell, _ in tiling:
        ...
        if reversibly_deletable:
            ...
            tilings = [ Tiling(new_tiling_dict), blocks ]
            yield RecursiveStrategy(formal_step, tilings, [tiling._back_map for tiling in tilings])
            for recursive_strategy in reversibly_deletable_cells_joined(tiling, basis, cell, new_occurrences_by_perm, path):
                yield recursive_strategy
            path.pop()
