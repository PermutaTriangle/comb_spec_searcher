...
class StrategyPack:
    def __init__(
        ...
        **kwargs
        ...
        if initial_strats is None:
            raise TypeError(
                (
                    "Strategy pack requires a (possibly empty) list "
    ...
    def __str__(self):
        ...
        initial_strats = ", ".join(get_func_name(f) for f in self.initial_strats)
        infer_strats = ", ".join(get_func_name(f) for f in self.inferral_strats)
        verif_strats = ", ".join(get_func_name(f) for f in self.ver_strats)
        string += "Inferral: {}\n".format(infer_strats)
